 ...
 * of <CODE>Section</CODE> to the
 * <CODE>Chapter</CODE> or <CODE>Section</CODE> to which you want to
 * add the new <CODE>Section</CODE>.
 * <p>
 * Example:
 * <BLOCKQUOTE><PRE>
 * Paragraph title2 = new Paragraph("This is Chapter 2", FontFactory.getFont(FontFactory.HELVETICA,
 * 18, Font.BOLDITALIC, new Color(0, 0, 255))); Chapter chapter2 = new Chapter(title2, 2); Paragraph
 * someText = new Paragraph("This is some text"); chapter2.add(someText); Paragraph title21 = new
 * Paragraph("This is Section 1 in Chapter 2", FontFactory.getFont(FontFactory.HELVETICA, 16,
 * Font.BOLD, new Color(255, 0, 0)));
 * <STRONG>Section section1 = chapter2.addSection(title21);</STRONG>
 * Paragraph someSectionText = new Paragraph("This is some silly paragraph in a chapter and/or
 * section. It contains some text to test the functionality of Chapters and Section.");
...
public class Section extends ArrayList<Element> implements LargeElement {
    // constant
    /**
     ...
     * @since iText 2.0.8
     */
    public static Paragraph constructTitle(Paragraph title, java.util.List<Integer> numbers,
    ...
    protected static void processSection(Section pdfSection,
            com.justifiedsolutions.openpdf.pdf.Section modelSection) {
        pdfSection.setTriggerNewPage(modelSection.isStartsNewPage());
        if (modelSection.isDisplaySectionNumber()) {
            pdfSection.setNumberStyle(NUMBERSTYLE_DOTTED_WITHOUT_FINAL_DOT);
        } else {
            pdfSection.setNumberDepth(0);
        }
        for (Content content : modelSection.getContent()) {
            addContent(pdfSection, content);
        }
        for (com.justifiedsolutions.openpdf.pdf.Section child : modelSection.getSections()) {
            addSection(pdfSection, child);
        }
    }

    protected static void addSection(Section parent,
            com.justifiedsolutions.openpdf.pdf.Section section) {
        Objects.requireNonNull(parent);
        Objects.requireNonNull(section);
        Paragraph title = Paragraph.getInstance(section.getTitle());
        Section result = parent.addSection(title);
        processSection(result, section);
    }

    protected static void addContent(Section section, Content content) {
        Objects.requireNonNull(section);
        Objects.requireNonNull(content);
        if (content instanceof com.justifiedsolutions.openpdf.pdf.content.Paragraph) {
            section.add(Paragraph
                    .getInstance((com.justifiedsolutions.openpdf.pdf.content.Paragraph) content));
        } else if (content instanceof com.justifiedsolutions.openpdf.pdf.content.Phrase) {
            section.add(Phrase.getInstance(
            ...
            section.add(
                    Chunk.getInstance((com.justifiedsolutions.openpdf.pdf.content.Chunk) content));
        } else if (content instanceof com.justifiedsolutions.openpdf.pdf.content.Table) {
            section.add(PdfPTable.getInstance((Table) content));
        }
    }
     ...
     * @param o     an object of type <CODE>Paragraph</CODE>, <CODE>List</CODE> or
     *              <CODE>Table</CODE>=
    ...
    public void add(int index, Element o) {
        ...
        try {
            ...
            if (element.isNestable()) {
                ...
                throw new ClassCastException(MessageLocalization
                        .getComposedMessage("you.can.t.add.a.1.to.a.section",
                                element.getClass().getName()));
            }
        } catch (ClassCastException cce) {
            throw new ClassCastException(MessageLocalization
                    .getComposedMessage("insertion.of.illegal.element.1", cce.getMessage()));
        }
    }
     ...
     * section will be numbered with their own number. If the numberdepth is higher (for instance
     * {@literal x > 1}), the numbers of {@literal x - 1} parents will be shown.
     *
