...
import static com.justifiedsolutions.openpdf.text.error_messages.MessageLocalization.getComposedMessage;

import com.justifiedsolutions.openpdf.text.Anchor;
import com.justifiedsolutions.openpdf.text.Annotation;
import com.justifiedsolutions.openpdf.text.BadElementException;
...
import com.justifiedsolutions.openpdf.text.Header;
 ...
 * Example:
 * <BLOCKQUOTE><PRE>
 * // creation of the document with a certain size and certain margins Document document = new
 * Document(PageSize.A4, 50, 50, 50, 50); try { // this will write HTML to the Standard
 * OutputStream
 ...
 * } catch(DocumentException de) { System.err.println(de.getMessage()); } // this will close the
 * document and all the OutputStreams listening to it
...
public class HtmlWriter extends DocWriter {
    ...
    public boolean add(Element element) throws DocumentException {
        ...
        if (open && !element.isContent()) {
            throw new DocumentException(getComposedMessage(
        ...
        try {
            switch (element.type()) {
                ...
                case Element.CREATIONDATE:
                    writeComment(
                            "Creationdate: " + HtmlEncoder.encode(((Meta) element).getContent()));
                    return true;
                case Element.MARKED:
    ...
    public void open() {
        ...
        try {
            ...
            if (document.getJavaScript_onLoad() != null) {
                write(HtmlTags.JAVASCRIPT_ONLOAD,
                        HtmlEncoder.encode(document.getJavaScript_onLoad()));
            }
            if (document.getJavaScript_onUnLoad() != null) {
                write(HtmlTags.JAVASCRIPT_ONUNLOAD,
                        HtmlEncoder.encode(document.getJavaScript_onUnLoad()));
            }
        ...
        switch (element.type()) {
            ...
            case Element.PHRASE: {
                ...
                if (phrase.hasLeading()) {
                    styleAttributes
                            .setProperty(Markup.CSS_KEY_LINEHEIGHT, phrase.getLeading() + "pt");
                }
            ...
            case Element.ANCHOR: {
                ...
                if (anchor.hasLeading()) {
                    styleAttributes
                            .setProperty(Markup.CSS_KEY_LINEHEIGHT, anchor.getLeading() + "pt");
                }
    ...
    protected void writeSection(Section section, int indent) throws IOException {
        if (section.getTitle() != null) {
            ...
            if (section.getTitle().hasLeading()) {
                styleAttributes.setProperty(Markup.CSS_KEY_LINEHEIGHT,
                        section.getTitle().getTotalLeading() + "pt");
            }
            // start tag
            addTabs(indent);
