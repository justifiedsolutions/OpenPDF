...
import com.justifiedsolutions.openpdf.text.pdf.CMapAwareDocumentFont;
...
public class ParsedText extends ParsedTextImpl {
     ...
     * <p>
     * Decodes a String (which will contain glyph ids encoded in the font's encoding) based on the
     * active font. This is supported for compatibility, but is no longer preferred.
     *
     * @param text          string
    ...
    /**
     * Internal constructor for a parsed text item. The constructors that call it gather some
     * information from the graphical state first.
     *
     * @param text          This is a PdfString containing code points for the current font, not
     *                      actually characters. If the font has multiByte glyphs, (Identity-H
     *                      encoding) we reparse the string so that the code points don't get split
     *                      into multiple characters.
     * @param graphicsState graphical state
     * @param textMatrix    transform from text space to graphics (drawing space)
     * @param unscaledWidth width of the space character in the font.
     */
    private ParsedText(PdfString text, GraphicsState graphicsState, Matrix textMatrix,
            float unscaledWidth) {
     ...
     */
    @Deprecated
    private ParsedText(String text, GraphicsState graphicsState, Matrix textMatrix,
            float unscaledWidth) {
    ...
    /**
     * Calculates the width of a space character. If the font does not define a width for a standard
     * space character , we also attempt to use the width of \u00A0 (a non-breaking space in many
     * fonts)
     *
    ...
    private static float getStringWidth(String string, GraphicsState graphicsState) {
        ...
        for (char c : chars) {
            ...
            totalWidth += (w * graphicsState.getFontSize() + graphicsState.getCharacterSpacing()
                    + wordSpacing)
     ...
     * @param hasSpace array same length as chars, each position representing whether it breaks a
     *                 word
     ...
     * @param baseline           baseline of this word, so direction of progress can be measured in
     *                           line ending determination.
     * @param wordsAreComplete   true means characters in this word won't be split apart
     *                           graphically
     * @param currentBreakBefore true if this word fragment represents a word boundary, and any
     *                           preceding fragment is complete.
     * @return the new word
     */
    private Word createWord(StringBuffer wordAccum,
        ...
        return new Word(graphicsState.getFont().decode(wordAccum.toString()), getAscent(),
                getDescent(),
    ...
    /**
     * @see TextAssemblyBuffer#getFinalText(PdfReader,
    ...
    public String toString() {
        return "[ParsedText: [" + getText() + "] " + getStartPoint() + ", " + getEndPoint()
                + "] lead" + "]";
    }

    /**
     * @see ParsedTextImpl#shouldNotSplit()
     */
    @Override
    ...
    /**
     * @return
     * @see ParsedTextImpl#breakBefore()
     */
    @Override
