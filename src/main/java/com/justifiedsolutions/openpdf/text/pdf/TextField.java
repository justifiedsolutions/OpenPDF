...
/**
 * Supports text, combo and list fields generating the correct appearances. All the option in the
 * Acrobat GUI are supported in an easy to use API.
 *
...
public class TextField extends BaseField {
    ...
    public PdfAppearance getAppearance() throws IOException, DocumentException {
        ...
        if ((options & MULTILINE) != 0) {
            ...
            float factor = ufont.getFontDescriptor(BaseFont.BBOXURY, 1) - ufont
                    .getFontDescriptor(BaseFont.BBOXLLY, 1);
            ...
            ct.setSimpleColumn(extraMarginLeft + 2 * offsetX, -20000, box.getWidth() - 2 * offsetX,
                    offsetY + leading);
            ...
            if (usize == 0) {
                float maxCalculatedSize = h / (ufont.getFontDescriptor(BaseFont.BBOXURX, 1) - ufont
                        .getFontDescriptor(BaseFont.BBOXLLY, 1));
                ...
                if (wd == 0) {
                    ...
                    usize = Math.min(maxCalculatedSize,
                            (box.getWidth() - extraMarginLeft - 4 * offsetX) / wd);
                }
                if (usize < 4) {
                    usize = 4;
                }
            ...
            changeFontSize(phrase, usize);
            float offsetY = offX +
                    ((box.getHeight() - 2 * offX) - ufont.getFontDescriptor(BaseFont.ASCENT, usize))
                            / 2;
            if (offsetY < offX) {
                offsetY = offX;
            }
            ...
            if ((options & COMB) != 0 && maxCharacterLength > 0) {
                ...
                for (Object o : phrase) {
                    ...
                    for (int j = 0; j < sb.length(); ++j) {
                        ...
                        app.setTextMatrix(extraMarginLeft + start - wd / 2,
                                offsetY - extraMarginTop);
                        app.showText(c);
                        start += step;
                    }
                }
                ...
                ColumnText.showTextAligned(app, alignment, phrase, x, offsetY - extraMarginTop, 0,
                        rtl, 0);
            }
        }
        app.restoreState();
    ...
    PdfAppearance getListAppearance() throws IOException, DocumentException {
        ...
        float leading = ufont.getFontDescriptor(BaseFont.BBOXURY, usize) - ufont
                .getFontDescriptor(BaseFont.BBOXLLY, usize);
        ...
        topFirst = first;
        app.saveState();
        app.rectangle(offsetX, offsetX, box.getWidth() - 2 * offsetX,
                box.getHeight() - 2 * offsetX);
        app.clip();
        app.newPath();
        Color fcolor = (textColor == null) ? GrayColor.GRAYBLACK : textColor;
        ...
        for (Object choiceSelection : choiceSelections) {
            ...
            if (curChoice >= first && curChoice <= last) {
                app.rectangle(offsetX, offsetX + h - (curChoice - first + 1) * leading,
                        box.getWidth() - 2 * offsetX, leading);
                app.fill();
            }
        }
        float xp = offsetX * 2;
        ...
        for (int idx = first; idx < last; ++idx, yp -= leading) {
            ...
            int rtl =
                    checkRTL(ptext) ? PdfWriter.RUN_DIRECTION_LTR : PdfWriter.RUN_DIRECTION_NO_BIDI;
    ...
    public PdfFormField getTextField() throws IOException, DocumentException {
        ...
        field.setBorderStyle(
                new PdfBorderDictionary(borderWidth, borderStyle, new PdfDashPattern(3)));
    ...
    protected PdfFormField getChoiceField(boolean isList) throws IOException, DocumentException {
        ...
        if (choiceExports == null) {
            if (isList) {
                ...
                field = PdfFormField
                        .createCombo(writer, (options & EDIT) != 0, uchoices, topChoice);
            }
        } else {
    ...
    /**
     * Sets the export values in list/combo fields. If this array is <CODE>null</CODE> then the
     * choice values will also be used as the export values.
     *
    ...
    /**
     * adds another (or a first I suppose) selection to a MULTISELECT list. This doesn't do anything
     * unless {@code this.options & MUTLISELECT != 0}
     *
    ...
    /**
     * replaces the existing selections with the param. If this field isn't a MULTISELECT list, all
     * but the first element will be removed.
     *
    ...
    /**
     * Gets the extensionFont. This font will be searched before the substitution fonts. It may be
     * <code>null</code>.
     *
    ...
    /**
     * Sets the extensionFont. This font will be searched before the substitution fonts. It may be
     * <code>null</code>.
     *
