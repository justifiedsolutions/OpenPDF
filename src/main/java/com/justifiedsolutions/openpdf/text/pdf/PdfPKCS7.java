...
public class PdfPKCS7 {
    ...
    static {
        ...
        digestNames.put("1.2.840.10040.4.3",
                "SHA1");    // TODO: bug - duplicate key - overwrites this with DSA
        digestNames.put("2.16.840.1.101.3.4.3.1",
                "SHA224");  // TODO: bug - duplicate key - overwrites this with DSA
    ...
    public PdfPKCS7(byte[] contentsKey, byte[] certsKey, String provider) {
        try {
            ...
            Collection<Certificate> certificates = certificateFactory
                    .engineGenerateCertificates(new ByteArrayInputStream(certsKey));
    ...
    public PdfPKCS7(byte[] contentsKey, String provider) {
        try {
            ...
            ASN1Sequence content = (ASN1Sequence) ((ASN1TaggedObject) signedData.getObjectAt(1))
                    .getObject();            // the positions that we care are:
            ...
            CertificateFactory certificateFactory = new CertificateFactory();
            Collection<Certificate> certificates = certificateFactory
                    .engineGenerateCertificates(new ByteArrayInputStream(contentsKey));
            ...
            if (rsaData.size() > 1) {
                ASN1OctetString rsaDataContent = (ASN1OctetString) ((ASN1TaggedObject) rsaData
            ...
            if (next < signerInfo.size() && (signerInfo
                    .getObjectAt(next) instanceof ASN1TaggedObject)) {
            ...
            if (RSAdata != null || digestAttr != null) {
                if (provider == null || provider.startsWith("SunPKCS11")) {
                    messageDigest = MessageDigest
                            .getInstance(getStandardJavaName(getHashAlgorithm()));
                } else {
                    messageDigest = MessageDigest
                            .getInstance(getStandardJavaName(getHashAlgorithm()),
                                    provider);
    ...
    public PdfPKCS7(PrivateKey privKey, Certificate[] certChain, CRL[] crlList,
        ...
        if (hasRSAdata) {
            ...
            if (provider == null || provider.startsWith("SunPKCS11")) {
                ...
                messageDigest = MessageDigest
                        .getInstance(getStandardJavaName(getHashAlgorithm()), provider);
            }
        }
    ...
    public boolean verifyTimestampImprint() throws NoSuchAlgorithmException {
        ...
        byte[] md = MessageDigest.getInstance(getStandardJavaName(getDigest(algOID)))
                .digest(digest);
     ...
     * @param digestEncryptionAlgorithm the encryption algorithm. It may must be <CODE>null</CODE>
     *                                  if the
